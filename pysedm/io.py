...
def filename_to_time(filename):
    """ """
    date, hms = filename.split("sedm2_")[-1].split("_")[0:2]
    return Time(f'{date[0:4]}-{date[4:6]}-{date[6:8]}T{hms[0:2]}:{hms[2:4]}:{hms[4:]}',format='isot')

def filename_to_date(filename, iso=False):
    """ """
    if iso:
        return filename_to_time(filename).datetime.isoformat().split("T")[0]
    return filename.split("sedm2_")[-1].split("_")[0]
...
def filename_to_guider(filename, astrom=True, extinction=".fits", nomd5=True):
    ...
    pointfiles = glob(f'{REDUXPATH}/phot/{fileinfo["date"]}/{key}*{fileinfo["date"]}*{fileinfo["name"]}*.fits')
    last_pointfile = None
    print(pointfiles)
